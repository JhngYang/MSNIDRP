...
class eni(object):
    ...
    def save_embeddings(self,save_path=None,save_format='npy'): 
        print("Save embeddings in {}".format(save_path))
        embeddings = self.get_embeddings()
        if save_format=='csv' :
            filename = os.path.join(save_path,self.args.embeddings_prefix+'embeddings.csv')
            np.savetxt(filename,embeddings,delimiter=',')
        elif save_format=='mat' :
            filename = os.path.join(save_path,self.args.embeddings_prefix+'embeddings.mat')
            # np.save(filename, embeddings)
            scio.savemat(filename,{'embeddings',embeddings})  
        elif save_format=='npy' :
            filename = os.path.join(save_path,self.args.embeddings_prefix+'embeddings.npy')    
            np.save(filename, embeddings)                               
